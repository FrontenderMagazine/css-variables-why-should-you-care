...
Chrome начиная 49-й версии. Они могут быть полезными для уменьшения повторяющегося кода в CSS, а
также для эффектов вроде смены тем на лету и, потенциально, для
...
которые будут использоваться в процессе разработки, чтобы поддержать единый стиль. Увы, многократное использование этих цветов в CSS не только является рутиной, но ещё и повышает предрасположенность к ошибкам. Если в какой-то момент
один из цветов нужно будет поменять, разработчик может неосторожно воспользоваться
«find-and-replace» по всему документу, что в большом проекте
может быть довольно опасно.

В последнее время многие разработчики стали использовать CSS-препроцессоры типа
SASS или LESS, которые решают эту проблему через переменные
препроцессора. Хотя эти инструменты заметно увеличили продуктивность разработки,
препроцессорные переменные имеют очень серьёзный недостаток: они статичны и не
могут меняться на лету. Появление возможности динамически менять переменные не
только позволит динамически менять темы сайта или приложения, но также означает значительное расширение возможностей отзывчивого дизайна и возможность создания полифилов для будущих свойств CSS. С выходом Chrome 49 это стало доступно в виде кастомных CSS-свойств.

## Кастомные свойства в двух словах

Кастомные свойства добавляют две новые возможности в наш CSS-инструментарий.

Например, автор кода может присваивать произвольные значения свойствам с придуманными им самим
именами. Функция `var()` позволяет затем использовать эти значения в других свойствах.
...
результате чего получается `color: #06c;`. Если кастомное свойство где-то определено в вашей таблице стилей, оно будет доступно функции `var`.
...
Это было сделано специально для максимальной гибкости и возможности в перспективе создавать макросы для `$foo`. Более подробно можно
почитать в [статье][1] одного из авторов спецификации, Таба Аткинса.
...
Обратите внимание, что кастомные свойства регистрозависимы, то есть `--header-color` и `--Header-Color` — это два разных свойства. Хотя синтаксис кастомных свойств поначалу может показаться незамысловатым, на самом деле он позволяет сделать довольно много. К примеру, ниже — пример валидного кастомного свойства:

    --foo: if(x > 5) this.width = 10;

Хотя это выражение не будет работать в качестве переменной (а также не будет валидным для любого обычного свойства), потенциально оно может быть прочитано и распознано на лету с помощью JavaScript. Это означает, что кастомные свойства могут открыть
доступ ко всевозможным интересным техникам, недоступным с нынешними
CSS-препроцессорами. Так что если вы, позевывая, думаете что-то вроде «Какая
...
Это означает, что можно использовать кастомные свойства в медиавыражениях для управления отзвычивым дизайном. Примером использования может служить увеличение внешних отступов от основных структурных элементов при увеличении размеров экрана:
...
Необходимо отметить, что вышеприведённый приём невозможно повторить на CSS-препроцессорах, потому что они неспособны переопределять переменные внутри медиавыражений.
...
внешних и внутренних отступов) разделяется не запятыми, а пробелами, так что валидный
...
небольшой удобный инструмент, позволяющий выполнять вычисления, чтобы определять значения в CSS. Она [поддерживается всеми современными браузерами][2], и может
